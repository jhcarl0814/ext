...
class concat_view : public view_interface<concat_view<Views...>> {
        ...
        class iterator: public xo::iter_cat_base_t<Const, Views...>
        {
          ...
          public:
            ...
            {
                xo::visit_i(it_, [this](auto I, auto &&)
                    { this->prev<I>(); });
                return *this;
            }
            ...
            friend constexpr bool operator==(iterator const &it, default_sentinel_t const &)
            {
                ...
                requires xo::concat_is_random_access<Const, Views...>
            {
                ...
                if(ix > iy) {
                    ...
                    auto const all_sizes = std::apply(
                        [&](auto const &...views)
                        {
            ...
            friend constexpr difference_type operator-(iterator const &it, default_sentinel_t)
                requires(xo::concat_is_random_access<Const, Views...> &&
                    sized_range<xo::back<__maybe_const<Const, Views>...>>)
            {

                auto const idx = it.it_.index();
                auto const all_sizes = std::apply(
                    [&](auto const &...views)
                    {
            ...
            friend constexpr difference_type operator-(default_sentinel_t, iterator const &it)
                requires(xo::concat_is_random_access<Const, Views...> &&
                    sized_range<xo::back<__maybe_const<Const, Views>...>>)
            {
            ...
            friend constexpr decltype(auto) iter_move(iterator const &ii) noexcept(
                ((std::is_nothrow_invocable_v<decltype(ranges::iter_move),
                      iterator_t<__maybe_const<Const, Views>> const &> &&
      ...
      public:
        ...
        constexpr iterator<true> begin() const
            requires((range<Views const> && ...) && xo::concatable<const Views...>) //
        {
        ...
        constexpr auto size() const
            requires(sized_range<Views const> && ...)
        {
            return apply(
                [](auto... sizes)
                {
